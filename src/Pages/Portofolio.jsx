...
import { db } from "../firebase"; // only db from firebase.js
...
export default function FullWidthTabs() {
  ...
  const fetchData = useCallback(async () => {
    try {
      // fetch Firestore collections dynamically
      ...
      const certificateData = certificateSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    ...
    <div className="md:px-[10%] px-[5%] w-full sm:mt-0 mt-[3rem] bg-[#030014] overflow-hidden" id="Portofolio">
      ...
      <div className="text-center pb-10" data-aos="fade-up" data-aos-duration="1000">
        ...
        <p className="text-slate-400 max-w-2xl mx-auto text-sm md:text-base mt-2">
          Explore my journey through services, certifications, and technical expertise. 
      ...
      <Box sx={{ width: "100%" }}>
        <AppBar position="static" elevation={0} sx={{ bgcolor: "transparent", border: "1px solid rgba(255, 255, 255, 0.1)", borderRadius: "20px", position: "relative", overflow: "hidden" }}>
          <Tabs value={value} onChange={handleChange} textColor="secondary" indicatorColor="secondary" variant="fullWidth" sx={{ minHeight: "70px", "& .MuiTab-root": { fontSize: { xs: "0.9rem", md: "1rem" }, fontWeight: "600", color: "#94a3b8", textTransform: "none", transition: "all 0.4s", padding: "20px 0", borderRadius: "12px", "&:hover": { color: "#fff", backgroundColor: "rgba(139, 92, 246, 0.1)", transform: "translateY(-2px)" }, "&.Mui-selected": { color: "#fff", background: "linear-gradient(135deg, rgba(139, 92, 246, 0.2), rgba(59, 130, 246, 0.2))", boxShadow: "0 4px 15px -3px rgba(139, 92, 246, 0.2)" } } }}>
